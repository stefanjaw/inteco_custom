...
class ProductUseVariant(models.TransientModel):
    ...
    def new_action_create_new_variant(self):
        ...
        if product.default_code.find("ENM") >= 0:
            new_default_code = product.default_code.split("/ENM"[0])
            new_code = str(new_default_code[0]) +'/'+ 'ENM' + ' ' + str(product.edition_con) + ':' + str(approval_year)
        else:
            product.edition_con = 1
            new_code = str(product.default_code) + '/' + 'ENM' + ' ' + str(product.edition_con) + ':' + str(approval_year)
